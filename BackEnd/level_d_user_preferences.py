...
def show_text():
    global current_text_color, display_color
    canvas.delete("all")
    current_text_color = random.choice(colors)
    display_color = random.choice([color for color in colors if color != current_text_color])
    canvas.create_text(300, 200, text=current_text_color.upper(), fill=display_color, font=('Helvetica', 32))
    global start_time
...
def record_response(selected_color):
    ...
    correct = selected_color == current_text_color
...
def setup_keyboard():
    def on_key(event):
        # Map of key presses to colors based on the first letter of each color
        key_to_color = {
            'r': 'red',
            'b': 'blue',
            'g': 'green',
...
root = tk.Tk()
root.title("Level D - Mismatching Text and Color")
